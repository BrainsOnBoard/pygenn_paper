...
\begin{figure}
    ...
    \todo{thomas: update the numbers to be more some fraction of N or something.}}
...
While the cortical microcircuit model described in the previous section demonstrates many features of GeNN, it can implemented entirely with neuron and synapse models built into GeNN.
In this section 
...
stdp_model = create_custom_weight_update_class(
    ...
    param_names=["tauPlus", "tauMinus", "lambda", "alpha", "mu", "denDelayStep"],
    derived_params=[
        ("decayPlus", create_dpf_class(lambda pars, dt: np.exp(-dt / pars[0]))()),
        ("decayMinus", create_dpf_class(lambda pars, dt: np.exp(-dt / pars[1]))())],
    ...
    learn_post_code=
        ...
        if (dt > 0) {
            ...
            $(g) += $(lambda) * pow($(g), $(mu)) * timing;
        }
        """,

    pre_spike_code="$(preTrace) += 1.0;",
    pre_dynamics_code="$(preTrace) *= $(decayPlus);",
    
    post_spike_code="$(postTrace) += 1.0;",
    post_dynamics_code="$(postTrace) *= $(decayMinus);",
