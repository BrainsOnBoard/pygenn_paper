...
GeNN handles much of the complexity of using CUDA directly and automatically performs device-specific optimizations so as to to maximize performance.
...
For example, in the model presented above, we chose to implement the neuron using the idiomatic forward Euler integration scheme employed by \citet{Izhikevich2003a}.
...
While there are numerous ways to solve equations~\ref{eq:izhikevich_v}~and~\ref{eq:izhikevich_u}~\citep{Humphries2007,Hopkins2015,Pauli2018}, we chose to use the idiomatic forward Euler integration scheme employed by \citet{Izhikevich2003a} in the original work~\citep{Izhikevich2007}.
...
\end{lstlisting}
%
Using this time, we can now calculate how much to decay $C_{ij}$ using the closed-form solution to equation~\ref{eq:izhikevich_stdp_c}:
%
\begin{lstlisting}       
...
\end{lstlisting}
%
Adding the synaptic weight $w_{ij}$ update described by equation~\ref{eq:izhikevich_stdp_w} requires two further additions to the model.
...
\citet{Mikaitis2018} showed how equation~\ref{eq:izhikevich_stdp_w} could be solved algebraically, allowing $w_{ij}$ to be updated in an event-driven manner with:
%
\begin{align}
...
However, as almost \num{20000} stimuli will be injected over the course of a \SI{1}{\hour} simulation, we can offload the stimulus delivery entirely to the GPU in order to reduce potential overheads, using the following slightly more complex model:
...
\begin{figure}[t!]
    ...
    \caption{Results of Pavlovian conditioning experiment.
             ...
             Mean rates are calculated by convolving the spike train with a spike density function with $\sigma=\SI{10}{\milli\second}$.\todo{thomas:is there a standard citation for this technique/is this a reasonable description?}}
...
\begin{figure}[t!]
    ...
    `GPU stim' indicates simulations using the GPU stimuli delivery model and `recording' indicates simulations where the new recording system is employed.}
...
\begin{figure}[t!]
    ...
    Times are taken from the fastest version of each model i.e. the microcircuit using the recording system and the Pavlovian conditioning model using the recording system and the GPU stimuli delivery.}
...
Although we only record the spiking activity during the first and last \SI{50}{\second}, using the recording system on these two systems still significantly improves the overall performance whereas, delivering stimuli on the GPU only provides a minimal improvement.
