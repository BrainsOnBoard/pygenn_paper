...
%
\begin{align}
    \frac{dV_i}{dt} = & 0.04 V_i^2 + 5V_i + 140 - U_i + I_{\text{syn}_{i}} + I_{\text{ext}_{i}} \\
...
The neural populations are connected recurrently, as shown in figure~\ref{fig:potjans_circuit}, with instantaneous current-based synapses:
%
\begin{align}
    I_{\text{syn}_{i}}(t) = & \sum_{i=0}^{n} w_{ij} \sum_{t_{j}}  \delta(t - t_{j}),
\end{align}
%
...
%
\begin{align}
    \frac{dC_{ij}}{dt} = & -\frac{C_{ij}}{\tau_c} + \text{STDP}(\Delta t)\delta(t - t_\text{pre/post}) \\
    \frac{dw_{ij}}{dt} = & -C_{ij}D_j
\end{align}
%
...
\begin{align}
    \text{STDP}(\Delta t) & = \
        \begin{cases}
          \exp\left(-\frac{\Delta t}{\tau_{+}}\right) & if\, \Delta t>0\\
          \exp\left(\frac{\Delta t}{\tau_{-}}\right) & if\, \Delta t\leq0
        \end{cases} \label{eq:stdp_pair}
\end{align}
%
where $\tau_{+}=\tau_{-}=\SI{20}{\milli\second}$.
Finally, each excitatory neuron has an additional variable $D_j$ which describes extracellular dopamine concentration:
%
\begin{align}
    \frac{D_{j}}{t} = & -\frac{D_{j}}{\tau_d} + \text{DA}(t)
\end{align}
%
