...
We describe the implementation of event-driven learning in GeNN in our previous work~\citep{Knight2018} 

The most efficient way to implement rules such as the three-factor STDP rule described above in GeNN is to

\begin{lstlisting}[language=python]
izhikevich_stdp_model = genn_model.create_custom_weight_update_class(
    "izhikevich_stdp",
    
    param_names=["tauPlus",  "tauMinus", "tauC", 
                 "aPlus", "aMinus", "wMin", "wMax"],
...
%
Because $w_{ij}$
\citet{Mikaitis2018} showed how equations~\ref{eq:izhikevich_stdp_c},~\ref{eq:izhikevich_stdp_w}~and~\ref{eq:izhikevich_stdp_d} could be algebraically integrated, allowing $C_{ij}$, $w_{ij}$ and $D_j$ to be updated using the following event-based updated:
%
\begin{align}
    C_{ij}(t) & = C_{ij}(t_{c}^{last}) e ^{-\frac{t - t_{c}^{last}}{\tau_c}},\label{eq:c_trace_solution} \\
    D_{j}(t) & = D_{j}(t_{d}^{last}) e ^{-\frac{t - t_{d}^{last}}{\tau_d}},\label{equ:d_trace_solution} \\
    \Delta w_{ij} & = \frac{C(t_{c}^{last}) D(t_{d}^{last})}{-\left(\frac{1}{\tau_c} + \frac{1}{\tau_d}\right)} \left(e ^{-\frac{t - t_{c}^{last}}{\tau_c}} e ^{-\frac{t - t_{d}^{last}}{\tau_d}} - e ^{-\frac{t^{last} - t_{c}^{last}}{\tau_c}} e ^{-\frac{t^{last} - t_{d}^{last}}{\tau_d}}\right).\label{eq:general_weight_update}
\end{align}
%
where $t_{c}^{last}$ and $t_{d}^{last}$ represent the last times at which $C_{ij}$ and $D_{j}$ where updated respectively.
